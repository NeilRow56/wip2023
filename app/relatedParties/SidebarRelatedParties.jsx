...
const SidebarRelatedParties = () => {
  const [showSidebarRelatedParties, setShowSidebarRelatedParties] =
    useState(true);

  return (
    <>
      {showSidebarRelatedParties ? (
        <button
          className="flex text-4xl text-white items-center cursor-pointer fixed left-28 xl:left-36 2xl:left-40 top-3 z-50"
          onClick={() =>
            setShowSidebarRelatedParties(!showSidebarRelatedParties)
          }
        >
          x
        </button>
      ...
      <div
        className={`top-0 left-0 w-[12vw] bg-blue-600  p-5  text-white fixed h-full z-40  ease-in-out duration-300 ${
          showSidebarRelatedParties ? "translate-x-0 " : "-translate-x-full"
        }`}
      >
...
export default SidebarRelatedParties;
