...
const SidebarTrialBalanceAndJournals = () => {
  const [showSidebarReview, setShowSidebarReview] = useState(true);
    ...
    <>
      {showSidebarReview ? (
        <button
          ...
          onClick={() => setShowSidebarReview(!showSidebarReview)}
        >
          x
        </button>
      ) : (
      ...
      <div
        className={`top-0 left-0 w-[12vw] bg-blue-600  p-5  text-white fixed h-full z-40  ease-in-out duration-300 ${
          showSidebarReview ? "translate-x-0 " : "-translate-x-full"
        }`}
      >
...
export default SidebarTrialBalanceAndJournals;
