...
const SidebarDrawings = () => {
  const [showSidebarDrawings, setShowSidebarDrawings] = useState(true);
    ...
    <>
      {showSidebarDrawings ? (
        <button
          ...
          onClick={() => setShowSidebarDrawings(!showSidebarDrawings)}
        >
          x
        </button>
      ) : (
      ...
      <div
        className={`top-0 left-0 w-[12vw] bg-blue-600  p-5  text-white fixed h-full z-40  ease-in-out duration-300 ${
          showSidebarDrawings ? "translate-x-0 " : "-translate-x-full"
        }`}
      >
...
export default SidebarDrawings;
